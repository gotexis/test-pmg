...
const parse = () => {
  let ocrProcessed = [...ocr]

  for (let i = 0; i < ocrProcessed.length; i++) {
    const line = ocr[i]
    if (line.includes('@') && line.includes('$')) {
      ocrProcessed[i - 2] += " " + line
      ocrProcessed[i] = undefined
    }
  }

  ocrProcessed = ocrProcessed.filter(i => i !== undefined)

  const result = []

  ocrProcessed.forEach(
    (line, i) => {
      const nextLine = ocrProcessed[i+1]
      if (!nextLine) return
      if (nextLine.match(/[0-9]+\.{1}[0-9]{2}/gm) && !nextLine.includes('$')) {
        // matched
